...
- **多 Provider 支持**: 统一抽象层支持 Anthropic Claude、OpenAI GPT、Google Gemini 系列模型
- **流式响应**: 实时显示 LLM 输出，支持 SSE 流式解析
- **强大的工具系统**: 7 个内置工具（Bash、Read、Write、Edit、Grep、Find、Ls）支持串行/并行执行
- **自定义 TUI 引擎**: 基于 crossterm 的差分渲染引擎，支持 Kitty 键盘协议、多行编辑器、Markdown 渲染
- **会话管理**: 支持会话保存、恢复、Fork、导出 HTML
- **模糊匹配**: 内置模糊搜索和自动补全
...
| Crate | 职责 | 主要功能 |
|-------|------|----------|
| **pi-ai** | LLM 统一 API 层 | Provider 注册系统、流式响应、模型管理、SSE 解析、JSON 流式解析 |
| **pi-tui** | TUI 引擎 | Component trait 系统、差分渲染、键盘输入（Kitty 协议）、多行编辑器、Markdown 渲染、模糊匹配 |
| **pi-agent** | Agent 核心 | AgentTool trait、事件系统、串行/并行工具执行、CancellationToken、Agent 循环 |
| **pi-coding-agent** | CLI 入口 | 参数解析、7 个内置工具、系统提示词、会话管理、配置系统 |
