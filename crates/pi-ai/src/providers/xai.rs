//! xAI (Grok) Chat Completions API Provider 实现
//!
//! 支持 xAI Chat Completions API (/v1/chat/completions) 的流式调用
//! xAI API 与 OpenAI 兼容，但有以下限制：
//! - 不支持 `store` 参数
//! - 不支持 `developer` 角色（始终使用 `system`）
//! - 不支持 `reasoning_effort` 参数
//! - 使用 `max_completion_tokens` 字段
//! - 不支持 `strict` 模式
...
/// xAI Chat Completions API Provider
...
impl XaiProvider {
    ...
    fn build_request_body(
        ...
        // max_completion_tokens (xAI 使用 max_completion_tokens，不是 max_tokens)
    ...
    async fn process_stream(
        ...
        let mut assistant_message = AssistantMessage::new(Api::Xai, model.provider.clone(), &model.id);
...
#[async_trait]
impl ApiProvider for XaiProvider {
    fn api(&self) -> Api {
        Api::Xai
    }
...
/// 转换消息为 xAI 格式
/// xAI 使用 OpenAI 格式，但始终使用 `system` 角色（不支持 `developer`）
fn convert_messages(
    _model: &Model,
    context: &Context,
    ...
    // System prompt - xAI 始终使用 "system" 角色
    ...
    for msg in &context.messages {
        match msg {
            Message::User(user_msg) => {
                let xai_msg = convert_user_message(user_msg, _model)?;
                messages.push(xai_msg);
            }
            Message::Assistant(assistant_msg) => {
                if let Some(xai_msg) = convert_assistant_message(assistant_msg)? {
                    messages.push(xai_msg);
                }
            }
            Message::ToolResult(tool_result) => {
                let xai_msg = convert_tool_result_message(tool_result)?;
                messages.push(xai_msg);
            }
        }
    }
...
/// 注册 xAI Provider
pub fn register() {
    let provider = std::sync::Arc::new(XaiProvider::new());
...
mod tests {
    ...
    async fn test_stream_text_response() {
        ...
        // xAI SSE 格式 - 与 OpenAI 兼容
        let sse_body = r#"data: {"id":"chatcmpl-123","object":"chat.completion.chunk","created":1677652288,"model":"grok-2","choices":[{"index":0,"delta":{"role":"assistant"},"finish_reason":null}]}

data: {"id":"chatcmpl-123","object":"chat.completion.chunk","created":1677652288,"model":"grok-2","choices":[{"index":0,"delta":{"content":"Hello"},"finish_reason":null}]}

data: {"id":"chatcmpl-123","object":"chat.completion.chunk","created":1677652288,"model":"grok-2","choices":[{"index":0,"delta":{"content":" world"},"finish_reason":null}]}

data: {"id":"chatcmpl-123","object":"chat.completion.chunk","created":1677652288,"model":"grok-2","choices":[{"index":0,"delta":{},"finish_reason":"stop"}]}
...
"#;
        ...
        let mut model = sample_model(Api::Xai, Provider::Xai);
    ...
    async fn test_stream_tool_call() {
        ...
        // 工具调用测试
        let sse_body = r#"data: {"id":"chatcmpl-456","object":"chat.completion.chunk","created":1677652288,"model":"grok-2","choices":[{"index":0,"delta":{"role":"assistant","tool_calls":[{"index":0,"id":"call_abc123","type":"function","function":{"name":"get_weather","arguments":"{\"location\":\"Paris\"}"}}]},"finish_reason":null}]}

data: {"id":"chatcmpl-456","object":"chat.completion.chunk","created":1677652288,"model":"grok-2","choices":[{"index":0,"delta":{},"finish_reason":"tool_calls"}]}
...
"#;
    ...
    #[tokio::test]
    async fn test_system_prompt_role() {
        // 验证 xAI 始终使用 "system" 角色
        let context = Context::new(vec![])
            .with_system_prompt("You are a helpful assistant");
        
        let model = sample_model(Api::Xai, Provider::Xai);
        let messages = convert_messages(&model, &context).unwrap();
        
        assert!(!messages.is_empty());
        let first_msg = messages.first().unwrap();
        assert_eq!(first_msg["role"].as_str().unwrap(), "system");
    }
}
